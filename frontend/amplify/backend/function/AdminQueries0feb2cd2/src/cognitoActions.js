...
async function adminResetUserPassword() {
  ...
  var params = {
    ...
    Username: "bbmcmann" /* required */,
    Permanent: true,
  };

  // console.log(`Attempting to update ${username} PASSWORD`);
  console.log(`Attempting to update ben PASSWORD`);

  try {
    await cognitoidentityserviceprovider.adminSetUserPassword(params).promise();
    // console.log(`Success updating ${username} PASSWORD`);
    console.log(`Success updating Ben PASSWORD`);
    return {
      message: `Success updating Ben PASSWORD`,
      // message: `Success updating ${username} PASSWORD`,
    };
  } catch (err) {
    console.log(err);
    throw err;
